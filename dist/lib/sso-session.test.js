...
import { parseCookies, fetchSSOSession, getEmailFromSession, validateSSOConfig, verifyJWTSession, validateSSOSession } from './sso-session.js';
...
describe('validateSSOConfig', ()=>{
    ...
    it('should throw for missing sessionUrl when jwt not configured', ()=>{
        ...
        expect(()=>validateSSOConfig(config)).toThrow('SSO sessionUrl is required when jwt verification is not configured');
    });
    it('should throw for missing jwt.secret when jwt is configured', ()=>{
        ...
        expect(()=>validateSSOConfig(config)).toThrow('SSO jwt.secret is required when jwt verification is configured');
    });
});
describe('verifyJWTSession', ()=>{
    ...
    it('should return null for malformed JWT', async ()=>{
        const result = await verifyJWTSession('not-a-valid-jwt', {
            secret
        });
        expect(result).toBeNull();
    });
...
describe('fetchSSOSession', ()=>{
    ...
    it('should return session data on success', async ()=>{
        ...
        expect(result).toEqual(mockSession);
        expect(global.fetch).toHaveBeenCalledWith('https://sso.example.com/session', expect.objectContaining({
            method: 'GET',
            headers: {
