...
describe('handleSSOLogin', ()=>{
    it('should find and return existing user', async ()=>{
        ...
        const result = await handleSSOLogin(mockPayload, config, session, mockReq);
    ...
    it('should throw when user not found and allowSignUp is false', async ()=>{
        ...
        const mockReq = new Request('https://example.com');
        await expect(// eslint-disable-next-line @typescript-eslint/no-explicit-any
        handleSSOLogin(mockPayload, config, session, mockReq)).rejects.toThrow('Sign-up is not allowed');
    });
    it('should throw when session has no email', async ()=>{
        ...
        const mockReq = new Request('https://example.com');
        await expect(// eslint-disable-next-line @typescript-eslint/no-explicit-any
        handleSSOLogin(mockPayload, config, session, mockReq)).rejects.toThrow('SSO session missing email');
    });
    it('should call onSuccess callback', async ()=>{
        ...
        const mockReq = new Request('https://example.com');
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        await handleSSOLogin(mockPayload, config, session, mockReq);
    ...
    it('should call onError callback on failure', async ()=>{
        ...
        const mockReq = new Request('https://example.com');
        await expect(// eslint-disable-next-line @typescript-eslint/no-explicit-any
        handleSSOLogin(mockPayload, config, session, mockReq)).rejects.toThrow('Database error');
