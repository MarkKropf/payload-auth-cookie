 ...
 */ export async function generateUserToken(payload, collectionSlug, userId) {
    try {
 ...
 */ export function getPayloadCookies(payload, collectionSlug, token) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Payload's collections object uses dynamic keys
    const collection = payload.collections[collectionSlug];
    if (!collection || !collection.config.auth) {
        throw new Error('Collection does not have auth enabled');
    ...
    const buildCookie = (cookieName)=>{
        ...
        const isSecure = authConfig.cookies?.secure !== undefined ? authConfig.cookies.secure : process.env.NODE_ENV === 'production';
        if (isSecure) {
    ...
    const cookieName = isAdminCollection ? `${cookiePrefix}-token` : `${cookiePrefix}-token-${collectionSlug}`;
 ...
 */ export function getPayloadCookie(payload, collectionSlug, token) {
 ...
 */ export function getExpiredPayloadCookies(payload, collectionSlug) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Payload's collections object uses dynamic keys
    const collection = payload.collections[collectionSlug];
    if (!collection || !collection.config.auth) {
        throw new Error('Collection does not have auth enabled');
    ...
    const buildExpiredCookie = (cookieName)=>{
        const attributes = [
            `${cookieName}=`,
            'HttpOnly',
            'Path=/',
 ...
 */ export function getExpiredPayloadCookie(payload, collectionSlug) {
    // Return the first cookie for backward compatibility
